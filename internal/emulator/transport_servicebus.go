...
func (t *serviceBusTransport) plan(target map[string]any) (delivery, error) {
	entity := mapString(target, "queueOrTopicName")
	if entity == "" {
		return delivery{}, errors.New("target names no queueOrTopicName")
	}
...
// closeGrace bounds the AMQP shutdown handshake, so a Ctrl-C is not held up by a broker
// that has already gone away.
const closeGrace = 5 * time.Second

// Close closes every sender and the client. The emulator calls it on shutdown so a
// long-running local session does not leak the AMQP connection.
