...
var (
	countRE = regexp.MustCompile(`\b(\d{3}) operations\b`)
	ofTheRE = regexp.MustCompile(`\b(\d{3}) of the \d{3} operations\b`)
)
...
// prose is every file in the repo that can state a count: documentation, and the
// Go comments that justify the three-tier design. Only trees that are not ours
// are skipped. Generated pages are deliberately scanned along with their sources
// — docs/guide holds hand-written pages next to skill-derived ones, and a gate
// that skipped the directory would miss every hand-written page in it.
func prose() []string {
